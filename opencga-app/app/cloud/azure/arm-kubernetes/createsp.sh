...
if [[ "$#" -ne 2 ]]; then
  ...
  if [ ! -f azuredeploy.parameters.json ]; then
    ...
    rgName=$(cat azuredeploy.parameters.json | jq -r '.parameters.rgPrefix.value')
  fi
...
echo "To deploy: "
echo "   ./deploy.sh --subscription \"$subscriptionName\" --service-principal-file \"${out}\" --azure-file <main-azuredeploy-parameters-json>"
